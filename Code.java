...
class Pairr {
    Integer a;
    List<Pair> b;
    Pairr(Integer i , List<Pair> listt) {
        a = i;
        b = new ArrayList();
        for (Pair p : listt) {
            b.add(p);
...
public class Main {
    ...
    static List<List<Pairr>> graph = new ArrayList(9);

    public static void addEdge(int frm, int to, List<Pair> weight) {
        graph.get(frm).add(new Pairr(to, weight));
        graph.get(to).add(new Pairr(frm, weight));
    }
    ...
    public static void shortestPathFaster(int S, int V, int choice, int D) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < graph.get(u).size(); i++) {
                int v = graph.get(u).get(i).a;
                int weight = Integer.MAX_VALUE;
                for (int j = 0; j < 2; j++) {
                    if (choice == 1 && graph.get(u).get(i).b.get(j).a != -1) {
                        weight = Math.min(weight, graph.get(u).get(i).b.get(j).a);
                    } else if (choice == 2 && graph.get(u).get(i).b.get(j).b != -1) {
                        weight = Math.min(weight, graph.get(u).get(i).b.get(j).b);
                    }
                }
                ...
                if (d[v] > d[u] + weight) {
                    d[v] = d[u] + weight;
                    if (choice == 1) {
                        cost[v] = cost[u] + Math.max(graph.get(u).get(i).b.get(0).b, graph.get(u).get(i).b.get(1).b);
                    } else if (choice == 2) {
                        cost[v] = cost[u] + Math.max(graph.get(u).get(i).b.get(0).a, graph.get(u).get(i).b.get(1).a);
                    }
                    path[v] = u;
                    if (!inQueue[v]) {
                        q.add(v);
